...
export default function Home() {
  ...
  async function playCustomMusic(source = musicUrl) {
    if (!soundEnabled || !source) return false;
    stopCustomMusic();
    try {
  ...
  }, [now, remindersEnabled, hydrated, todayTasks, today, snoozes]);
          ...
          <section className="reminder-card">
            ...
            <div className="music-picker">
              ...
              <div className="music-actions">
                ...
                <button className="music-upload-button" onClick={() => musicInputRef.current?.click()} type="button">
                  {musicUrl ? "更换" : "选择音乐"}
                </button>
              </div>
            </div>
            ...
            <p className="music-hint">不限制播放时长；音乐只保存在当前设备，文件大小受浏览器可用空间限制。</p>
            ...
            <div className="background-note">
              <strong>{isDesktop ? "后台提醒已开启" : "关于关闭页面"}</strong>
              <span>
                {isDesktop
